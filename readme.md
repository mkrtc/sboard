...
|:-------:|------------|----------------------------------------------------------------------------------------------|
|    0    | Provider   | Работают с внешними источниками (HTTP, сокеты), но не знают, зачем.                          |
|    1    | Repository | Используют провайдеры и возвращают сущности — модели данных.                                 |
|    2    | Entity     | Модели данных с методами, умеют работать с собой и репозиторием.                             |
|    3    | Service    | Бизнес-логика компонента, работают с репозиториями и сущностями.                             |
|    4    | Component  | Максимально тупые. Используют только методы сервиса и хуки. Всё что не UI — уходит в сервис. |
